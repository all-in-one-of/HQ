...
class k_cachefinder():
		...
		if cc.pluginInfo("pgYetiMaya",q=1,l=1):
			...
			for yeti in yetis:
				...
				if yeticache:
					...
					##########设置节点内容 及 加入到节点数据#############
					ktemp={'path':yeticache,'port':'.cacheFileName'}
					...
					if yetiTexs:
						...
						for yetiTex in yetiTexs:
							...
							if yetiTexfile:
								...
								##########设置节点内容 及 加入到节点数据#############
								ktemp={'path':yetiTexfile,'texNode':yetiTex}
								if self.kNodedate.has_key('kYetitex'):
									#一个maya节点内 如果有多个 Yeti贴图节点 用数组添加法
									if self.kNodedate['kYetitex'].has_key(yeti):
										self.kNodedate['kYetitex'][yeti].append(ktemp)
									else:
										self.kNodedate['kYetitex'].update({yeti:[ktemp]})
	...
	def k_checkShaveCache(self):
		if cc.pluginInfo("shaveNode",q=1,l=1):
			...
			##########设置节点内容 及 加入到节点数据#############
			ktemp={'path':shaveStat,'port':'.tmpDir'}
			if self.kNodedate.has_key('kshave_cache'):
				self.kNodedate['kshave_cache'].update({'shaveGlobals':[ktemp]})
			else:self.kNodedate['kshave_cache']={'shaveGlobals':[ktemp]}
			####################################################\
	...
	def k_checkMRmeshfiles(self):
		if cc.pluginInfo("Mayatomr",q=1,l=1):
			...
			for mrmesh in mrmeshs:
				...
				##########设置节点内容 及 加入到节点数据##############
				ktemp={'path':kmrmesh,'port'='.object_filename'}
				if self.kNodedate.has_key('kmrcacheFiles'):
					self.kNodedate['kmrcacheFiles'].update({mrmesh:[ktemp]})
				else:self.kNodedate['kmrcacheFiles']={mrmesh:[ktemp]}
				######################################################
			...
			for mrmesh in mrmeshs:
				...
				if kmrmesh:
					...
					##########设置节点内容 及 加入到节点数据##############
					ktemp={'path':kmrmesh,'port'='.miProxyFile'}
					if self.kNodedate.has_key('kmrcacheFiles'):
						self.kNodedate['kmrcacheFiles'].update({mrmesh:[ktemp]})
					else:self.kNodedate['kmrcacheFiles']={mrmesh:[ktemp]}
					######################################################
		...
		if cc.pluginInfo("vrayformaya",q=1,l=1):
			...
			for kvrmesh in kvrmeshs:
				...
				if krvrmesh:
					...
					#设置节点内容 及 加入到节点数据
					ktemp={'path':vrmesh,'port':'.fileName'}
					if self.kNodedate.has_key('kvrmeshfiles'):
						self.kNodedate['kvrmeshfiles'].update({kvrmesh:[ktemp]})
					else:self.kNodedate['kvrmeshfiles']={kvrmesh:[ktemp]}
			...
			for vrsettfile in vrsettfiles:
				...
				for k_stype in ['causticsFile','fileName','pmap_file','imap_fileName']:
					...
					if kvrsettfile and not kvrsettfile in self.kvrsettfiles:
						...
						##########设置节点内容 及 加入到节点数据##################
						ktemp={'path':kvrsettfile,'port':('.'+k_stype)}
						if self.kNodedate.has_key('kvrsettfiles'):
							self.kNodedate['kvrsettfiles'].update({vrsettfile:[ktemp]})
						else:self.kNodedate['kvrsettfiles']={vrsettfile:[ktemp]}
						###########################################################
						#
