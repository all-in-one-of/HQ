    ...
    def kpresetbuild(self):
        ...
        for i in self.checkBoxKeys:
            ...
            for preset in k_presets:
                if preset not in self.k_preset:
                    self.k_preset.append(preset)
        ...
        for i in range(len(self.k_preset)):
            self.k_presetbutton = QtGui.QPushButton(self.k_preset[i])
            self.k_presetbutton.setObjectName(self.k_preset[i]+'_preset')
            #添加tips
            self.k_presetbutton.setToolTip(ktip)
            try:
                #根据pos信息确定控件位置
