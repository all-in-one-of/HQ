...
class k_cachefinder():
	...
	def k_Nodedate2(self,plugin,node,port,path):
		##########设置节点内容 及 加入到节点数据#############
		ktemp={'path':path,'port':port}
		if self.kNodedate.has_key(plugin):
					...
					if yetiTexs:
						...
						for yetiTex in yetiTexs:
							...
							if yetiTexfile:
								...
								##########设置节点内容 及 加入到节点数据#############
								self.k_Nodedate2('kYetitex',yeti,yetiTex,yetiTexfile)
								#####################################################
	...
	def k_checkMRmeshfiles(self):
		if cc.pluginInfo("Mayatomr",q=1,l=1):
			...
			for mrmesh in mrmeshs:
				...
				##########设置节点内容 及 加入到节点数据#############
				self.k_Nodedate('kmrcacheFiles',mrmesh,'.object_filename',kmrmesh)
				#####################################################
	...
	def k_cachefiles(self):
		...
		for cachefile in cachefiles:
			if cachefile :
				if os.path.splitext(cachefile)[-1]=='.mcx' or os.path.splitext(cachefile)[-1]=='.mc' or os.path.splitext(cachefile)[-1]=='.xml':
