...
def pv2_setCam(knob, workingNode, workingNodeName):
	...
	gizmoAxisPath = "%s.p_camAxis" % workingNodeName
	...
	if selectedCam == "None" or successChecker == 0 :
		...
		camExpressionT = selectedCam + '.translate'
		...
		nuke.toNode(gizmoCamPath)['translate'].setExpression(camExpressionT)
		...
		#now see if there is an axis connected upstream from the camera?
		upstreamConnection = nuke.toNode(selectedCam).input(0)
		if upstreamConnection:		
			#there's something there. check if it's axis
			if upstreamConnection.Class() == "Axis2" :
				#it is an axis. setup
				upstreamName = upstreamConnection.name()
			
				#get Axis transform links
				axisExpressionT = upstreamName + '.translate'
				axisExpressionR = upstreamName + '.rotate'
				
				#use links to create expressions
				nuke.toNode(gizmoAxisPath)['translate'].setExpression(axisExpressionT)
				nuke.toNode(gizmoAxisPath)['rotate'].setExpression(axisExpressionR)
