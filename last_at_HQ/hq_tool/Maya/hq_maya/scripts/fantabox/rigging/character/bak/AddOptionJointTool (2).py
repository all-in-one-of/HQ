...
class optionUIClass(object):
	...
	def optionUI(cls,winName ="optionWinUI"):
		...
		cmds.text(l="",w=35)
		cmds.button(optionUIClass.button1,l="创建骨骼",w=120,h=40,c='optionUIClass.button1_command()')
		cmds.text(l="",w=20)
		cmds.text(l="",w=20)
		cmds.text(l="",w=35)
		cmds.button(optionUIClass.button1_1,l="重新定位骨骼",w=120,h=40,c='optionJointClass.option_LocatorJoint()')
		cmds.text(l="",w=20)
		cmds.text(l="",w=20)
		cmds.setParent("..")
		cmds.separator()
		...
		cmds.text(l="",w=35)
		cmds.button(optionUIClass.button2,l="关联辅助骨骼",w=120,h=40,c='optionJointClass.optionJointLine()')
		cmds.setParent("..")
		cmds.separator()
		cmds.text(l="设置...",w=10,h=30)
		cmds.rowColumnLayout(optionUIClass.setLayout,nbg=1,nc=5,h=50)
		cmds.text(l="",w=20)
		cmds.button(optionUIClass.buttonFollow1,h=35,w=70,l="十字骨骼设置",c = 'option_setUI()')
		cmds.text(l="",w=5)
		cmds.button(optionUIClass.buttonFollow2,h=35,w=70,l="选择蒙皮骨骼", c = 'Option_SelectToSkinJoint()')
		cmds.text(l="",w=10)
		cmds.setParent("..")
		cmds.separator()
		...
		cmds.text(l="",w=20)
		cmds.text(l="",w=35)
		cmds.button(optionUIClass.button3,l="添加ADV辅助骨骼",w=120,h=40,c='doItAddAdvOptionJoint()')
		cmds.text(l="",h=5)
		cmds.text(l="",h=5)
		cmds.text(l="",w=35)
		cmds.button(optionUIClass.button4,l="关联ADV辅助骨骼",w=120,h=40,c='doItAddAdvOptionJointLine()')
...
def advSetAttr(l,r,sli,e,attr):
	if cmds.objExists(r):
		...
		tf =eval('cmds.transformLimits('+'\"' + l + '\"' + ',q=1,'+ e + '=1)')
		eval('cmds.transformLimits('+'\"' + r + '\"' + ','+ e + '='+ str(tf) +')')

		tn =eval('cmds.transformLimits('+'\"' + l + '\"' + ',q=1,'+ attr + '=1)')
		eval('cmds.transformLimits('+'\"' + r + '\"' + ','+ attr + '='+ str(tn) +')')
...
def Option_MirrorAttr(obj_l,obj_r):
	...
	if "_joint2" in obj_l:
		tf =eval('cmds.transformLimits('+'\"' + obj_l + '\"' + ',q=1,etx=1)')
		f1 =eval('cmds.transformLimits('+'\"' + obj_l + '\"' + ',q=1,tx=1)')
		eval('cmds.transformLimits('+'\"' + obj_r + '\"' + ',etx='+ str(tf) +",tx=" + str(f1) +')')

	if "_joint1" in obj_l:
		tn =eval('cmds.transformLimits('+'\"' + obj_l + '\"' + ',q=1,erz=1)')
		f2 =eval('cmds.transformLimits('+'\"' + obj_l + '\"' + ',q=1,rz=1)')
		eval('cmds.transformLimits('+'\"' + obj_r + '\"' + ',erz='+ str(tn) +",rz=" + str(f2) +')')
