...
class CleanCheckRIG(QtGui.QDialog):
	...
	def __init__(self , parent = None):
		...
		self.checkBox_17 = QtGui.QCheckBox(unicode('手动清理按面模型' , 'gbk') , self.groupBox)
	...
	def checkNodesMain(self , *args):
		...
		while True:
			...
			amount += eachCheckListLength
			pm.progressWindow(e = True , progress = amount , status = 'Visibility=off的节点是否lock：')
			
			
			if self.checkBox_7.isChecked():
				LayouName = self.addFrm(self.checkObjectLock , '5' , 'QlookLayout' , 'Visibility=off的节点是否lock：' )
				pm.button(p = LayouName , l = '锁定' ,c = functools.partial(self.setObjectLock , LayouName) )

			amount += eachCheckListLength
			pm.progressWindow(e = True , progress = amount , status = '驱动过的物体是否锁定并设置成了不可K帧：')
			
			if self.checkBox_8.isChecked():
				LayouName = self.addFrm(self.checkObjectSetKey , '6' , 'QlookLayout' , '驱动过的物体是否锁定并设置成了不可K帧：' )
			...
			amount += eachCheckListLength
			pm.progressWindow(e = True , progress = amount , status = '手动清理按面模型')
			
			if self.checkBox_17.isChecked():
				LayouName = self.addFrm(self.cleanFaceFixedshader , '15' , 'QlookLayout' , '手动清理按面模型' )
				Frm = pm.frameLayout(LayouName , q = True , ca = True)[0]
				pm.textScrollList(Frm , e = True , append = '已清理( %s 个)按面模型'%(self.i))
	...
	def selObj(self , tex = None , *args):
		listObj = [obj for obj in cmds.textScrollList(tex,q=1,si=1) if cmds.objExists(obj)]
		cmds.select(listObj)
	...
	def checkObjectLock(self):
		...
		trans = [t for t in all if pm.nodeType(t.getShape()) != 'camera' if t.nodeType() != 'ikEffector']
	...
	def checkObjectSetKey(self):
		...
		transList = [t for t in all if t not in conList+ctrlList]
		attrs = ['t' , 'r' , 's'  ]
	...
	def checkMainCtrl(self):
		...
		if ctrl.hasAttr('showMod'):
			...
			if pm.objExists('shave_G') or pm.objExists('hair_G') or pm.objExists('yeit_G'):
				if len(ctrl.showMod.outputs()) <= 1:
					overText.append('{:s} : 没有关联毛发组'.format(ctrlName))
	...
	def setCheckMainCtrl(self , layou = None , *args):
		...
		if name:
			...
			if self.getGeoGroup():
				...
				for g in ['shave_G' , 'hair_G' , 'hair_G']:
					if pm.objExists(g):
						if g not in ctrl.showMod.outputs():
							ctrl.showMod.connect(g.v ,f = True)
	...
	def cleanFaceFixedshader(self):
		...
		mesh = [s.getParent() for s in multishape]
		return mesh
