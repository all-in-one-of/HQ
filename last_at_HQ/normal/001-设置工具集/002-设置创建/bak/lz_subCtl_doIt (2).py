...
#-----------------------------------------------------------------------------------------------
#
#	11.7.28完全改写lz_subCtl方法，精简功能，整合到目前架构中
#	11.8.5 扩充subCtl方法。
#	ctl_name表示 最后控制器的名字，当存在ctls时有效.
#
#-----------------------------------------------------------------------------------------------
	...
	#收集jointParent
		...
		for index,item in enumerate(ctl_joints):
			...
			if objExists(mir_name):
					mir_name = mir_name+'#'		
			joint_mirror = lz_subCtl_findMirrorObj(item,mirrorStrs)#找到存在的对称物体
			if joint_mirror: 
			#and (not mirrorStrs[1] or joint_mirror.find(mirrorStrs[1]) ==-1):#名字是否包含段字符串
			...
			if joint_parent:
				...
				if joint_parent in ctl_jParents and joint_parent not in ctl_joints:
					#print joint_parent
					mirror_parent = lz_subCtl_findMirrorObj(joint_parent,mirrorStrs)#找到存在的对称物体
					mirror_parent = joint_parent+'_mir' if not mirror_parent else mirror_parent#如果没有，就找mir后缀的物体
					if not objExists(mirror_parent):
						mirror_parent = lz_subCtl_mirrorObj(joint_parent,forceBhr=1)[0]#如果都没有，那就要mirror一个
						parent(joint_mirror,mirror_parent)
					#print mirror_parent
					temp_parent = listRelatives(joint_mirror,pa=1,p=1)
					if not temp_parent and mirror_parent !=joint_mirror:#当存在对称物体，就不需要parent
						parent(joint_mirror,mirror_parent)
...
def lz_subCtl_mirrorObj(obj='',name = '',forceBhr=-1):
	...
	if forceBhr!=-1:
		...
		if attributeQuery('jo',ex=1,node =obj):
			temp_jo = getAttr('%s.jo'%obj)[0]	#根据jo来判断是否根据behavior来对称
...
def lz_subCtl_Mirror():
	...
	if (res_str == prefix):
		lz_warning("要替换的文字并不存在！\\n")
		return 0
	if(objExists('res_str*')):
		lz_warning("镜像结果已存在，请先删除！\\n")
		return 0
...
def lz_subCtls_reLocateAll():
	...
	poly_tar = lz_ls()[0]
	if not poly_tar:
		lz_warning('请选择角色模型 并且上面有添加次级系统!\\n')
		return
	if not attributeQuery('subPoly',node = poly_tar,ex=1):
		lz_warning('请添加次级!\\n')
		return
...
def lz_polyPin_antiSmooth(object):
	...
	return 1
# 对于处理过的polyEdge节点，当smooth被删除时，使用该方法进行还原
# 调用方式同上。都是attach按钮	
...
def lz_ikfk_initTrans(objects=[],init= 1 ,data3 = [],notDo = ['_follow']):
	...
	for obj in objects:
		...
		if objectType(obj) != 'nurbsCurve':
			...
			attrs_all = []+attrs
			for attr in attrs:	
				temp_attr = attr+'_switch'#switch属性不可K，不在attrs里面，所以这样写
	...
	if skin_node:
		...
		for index,item in enumerate(inf_vtx):
			for sub_item in item:
				...
				#skinPercent(skin_node,componentV%(subText,sub_item),r=0,tv=(skin_object[index],1))
				for point_n in AllCVPoint:
						
						createNode("distanceBetween",name="MyDistanceNode")
						cv1=xform(cvpoint,q=1,ws=1,t=1)
						cv2=xform(point_n,q=1,ws=1,t=1)
						setAttr("MyDistanceNode.point1",cv1[0],cv1[1],cv1[2])
						setAttr("MyDistanceNode.point2",cv2[0],cv2[1],cv2[2])
						distance=getAttr("MyDistanceNode.distance")
						x=distance/ssd
						
						if x>1:
							skinPercent(skin_node, point_n,r=0,tv=(skin_object[index],0))
							print("skinPercent -tv " +skin_object[index]+" 0 "+ point_n +"\n")
						elif x==0:
							skinPercent(skin_node, point_n,r=0,tv=(skin_object[index],1))
						 	print("skinPercent -tv " +skin_object[index]+" 1 "+ point_n +"\n")
