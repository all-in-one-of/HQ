...
def creatConstrainPlane_window():
    ...
    mc.checkBox('getScaleConstrainObjCB',l='添加总控制器来缩放约束所有被约束对象',v=CBValue,cc='getCheckBoxValue()')
    mc.textFieldButtonGrp('ScaleConstrainGrp',l='总控制器：',bl='选择',en=CBValue,bc='getScaleConstrainObj()')
...
def getCheckBoxValue():
    global CBValue
    CBValue=mc.checkBox('getScaleConstrainObjCB',q=1,v=1)
    mc.textFieldButtonGrp('ScaleConstrainGrp',e=1,en=CBValue)
...
def getScaleConstrainObj():
    global ScaleConstrainObj
    ScaleConstrainObj=mc.ls(sl=1)[0]
    mc.textFieldButtonGrp('ScaleConstrainGrp',e=1,text=ScaleConstrainObj)
def ScaleConstrain():
    for one in ConstrainObjs:
        mc.scaleConstraint(ScaleConstrainObj,one,mo=1)
