    ...
    def checkAttribtypes(self, nodes, attrs):
        ...
        for a in attrs:
            ...
            while None in types:
                types.remove(None)
            if len(types)>1:
                self.par.parent.msg2(1, 'Attrib have different type and will be skipped:',a)
            elif len(types) == 0:
                self.par.parent.msg2(1, 'Attribute not found and will be skipped:',a)
            else:
                if types[0] in allTypes:
                    pyType = allTypes[types[0]]
                    if not attrs[a] is None:
                        try:
                            df = pyType(self.checkDefault(types[0],attrs[a]))
                            newAttrs[a] = [df, pyType]
                        except:
            ...
            if self.op['expgeogrp_obj'][0]:
                ...
                while not iter.isDone():
                    ...
                    if allPoly:
                        ...
                        data.append(allPoly)
                    iter.next()
                objGroups = self.convertObjectNamesToGroupArray(data, names,prefix+'_'+self.op['expgeogrp_obj'][2], self.op['expgeogrp_obj'][1],self.textVariables['noObject'])
                if objGroups:
...
#            PER SHAPE GROUPS
            if self.op['expgeogrp_shp'][0]:
                ...
                while not iter.isDone():
                    ...
                    iter.next()
                objGroups = self.convertObjectNamesToGroupArray(data, names,prefix+'_'+self.op['expgeogrp_shp'][2], self.op['expgeogrp_shp'][1],self.textVariables['noObject'])
                if objGroups:
                    self.primGroupsArray.update(objGroups)
                ...
                while not iter.isDone():
                    ...
                    for child in self.shapeGenerator(dagPath, om.MFn.kMesh):
                        ...
                        for i in range(shaders.length()):
                            ...
                            mats.append(self.checkLegalChar(prefix+'_'+self.op['expgeogrp_mtl'][2]+name))
                        ind = []
            ...
            if self.op['expgeogrp_parent'][0]:
                ...
                while not iter.isDone():
                    ...
                    for child in self.shapeGenerator(dagPath, om.MFn.kMesh):
                        ...
                        names.append(self.checkLegalChar(prefix+'_'+self.op['expgeogrp_parent'][2]+par))
                ...
                while not titer.isDone():
                    ...
                    if dagNode.childCount():
                        ...
                        if child.apiType() == om.MFn.kMesh:
                            ...
                            names.append(self.checkLegalChar(prefix+'_'+self.op['expgeogrp_layer'][2]+layer))
                    titer.next()
    ...
    def readCustomAttrib(self, iter):
        ...
        for i, node in enumerate(nodes):
            for attr in checkedAttrb:
                ...
                if not attr in self.primAttribArray:
                    self.primAttribArray[attr] = []
                value = self.getAttribFromNode(node, attr, attrType)
                if value is None:
                    value = default
                self.primAttribArray[attr] += ([value]*counts[i])
            ...
            if self.op['curvgrp_obj'][0]:
                ...
                while not iter.isDone():
                    ...
                    for child in self.shapeGenerator(dagPath, om.MFn.kNurbsCurve):
                        ...
                        data.append(1)
                    iter.next()
                objGroups = self.convertObjectNamesToGroupArray(data, names,prefix+'_'+self.op['curvgrp_obj'][2], self.op['curvgrp_obj'][1],self.textVariables['noCurveObj'])
                if objGroups:
                    self.primGroupsArray.update(objGroups)
            #parent group
            if self.op['curvgrp_par'][0]:
                ...
                while not iter.isDone():
                    ...
                    if isCurve:
                        ...
                        names.append(self.checkLegalChar(prefix+'_'+self.op['curvgrp_par'][2]+par))
                    iter.next()
                prntGroups = self.convertArrayDisplLayer(names, counts)
                if prntGroups:
                    self.primGroupsArray.update(prntGroups)
                ...
                while not titer.isDone():
                    ...
                    if dagNode.childCount():
                        ...
                        if child.apiType() == om.MFn.kNurbsCurve:
                            ...
                            names.append(self.checkLegalChar(prefix+'_'+self.op['curvgrp_layer'][2]+layer))
                    titer.next()
                displLayerGroups = self.convertArrayDisplLayer(names, counts)
                if displLayerGroups:
                    self.primGroupsArray.update(displLayerGroups)
    ...
    def parseGroups(self, iter):
        ...
        if self.pointCount():
            ...
            if self.op['exppartgrp_shape'][0]:
                ...
                objGroups = self.convertObjectNamesToGroupArray(data, names,prefix+'_'+self.op['exppartgrp_shape'][2], self.op['exppartgrp_shape'][1],self.textVariables['noObject'])
                if objGroups:
            ...
            if self.op['exppartgrp_emit'][0]:
                ...
                for k in emitGroups:
                    if emitGroups[k]:
                        resultEmitgroup[prefix+'_'+self.op['exppartgrp_emit'][2]+k]=emitGroups[k]
                if resultEmitgroup:
    ...
    def readAttributes(self, iter):
        ...
        if self.op['exppartattrib'][0]:
            ...
            while not iter.isDone():
                ...
                part = omx.MFnParticleSystem(dagPath)
                for atr in checked:
                    attrName = self.checkLegalChar(atr)
                    ...
                    if atr in ppAttrs: #from particles
                        if part.isPerParticleIntAttribute(atr) or part.isPerParticleDoubleAttribute(atr):
                            ...
                            for i in range(var.length()):
                                value.append(var[i])
                            if not attrName in self.defaultValues:
                                self.defaultValues[attrName] = 0
                            ...
                            if not attrName in self.defaultValues:
                                self.defaultValues[attrName] = default
                        else:
                            value = default
                    ...
                    if not atr in self.pointAttribArray:
                        ...
                        self.pointAttribArray[attrName] = offsArray + value
                    else:
                        self.pointAttribArray[attrName] += value
...
class pivotReader(geoCollector):
    ...
    def parseGroups(self, iter):
        ...
        if self.pointCount():
            ...
            if self.op['exppivgrp_obj'][0]:
                ...
                while not iter.isDone():
                    ...
                    data.append(1)
                    iter.next()
                objGroups = self.convertObjectNamesToGroupArray(data, names,prefix + '_'+self.op['exppivgrp_obj'][2], self.op['exppivgrp_obj'][1],self.textVariables['noObject'])
                if objGroups:
                    self.pointGroupsArray.update(objGroups)
                iter.reset()
            #PARENT GROUP
            if self.op['exppivgrp_parent'][0]:
                ...
                while not iter.isDone():
                    ...
                    names.append(self.checkLegalChar(prefix+'_'+self.op['exppivgrp_parent'][2]+par))
                    counts.append(1)
                    iter.next()
                prntGroups = self.convertArrayDisplLayer(names, counts)
                if prntGroups:
            ...
            if self.op['exppivgrp_shape'][0]:
                ...
                while not iter.isDone():
                    ...
                    data.append(1)
                    iter.next()
                objGroups = self.convertObjectNamesToGroupArray(data, names,prefix + '_'+self.op['exppivgrp_shape'][2], self.op['exppivgrp_shape'][1],self.textVariables['otherInstances'])
                if objGroups:
                    self.pointGroupsArray.update(objGroups)
                iter.reset()
            #DISPLAY LAYER GROUP
        if self.op['exppivgrp_layer'][0]:
            ...
            while not iter.isDone():
                ...
                names.append(self.checkLegalChar(prefix+'_'+self.op['exppivgrp_layer'][2]+layer))
                iter.next()
            displLayerGroups = self.convertArrayDisplLayer(names, counts)
            if displLayerGroups:
    ...
    def readCustomAttrib(self, iter):
        ...
        for i, node in enumerate(nodes):
            for attr in checked:
                if not attr in self.pointAttribArray:
                    self.pointAttribArray[attr] = []
                default = checked[attr][0]
                value = self.getAttribFromNode(node, attr, checked[attr][1], default)
                if value is None:
                    value = default
                self.defaultValues[attr] = default
                self.pointAttribArray[attr] += ([value])
