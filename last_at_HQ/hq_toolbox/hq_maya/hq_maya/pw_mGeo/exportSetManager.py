...
################################################################
    ...
    def refreshSets(self):
        ...
        sets = cmds.listSets(allSets=1)
        mgeoSets = []
        for s in sets:
            if not s in defaults and cmds.objExists(s):
                if cmds.attributeQuery( self.settingsAttribName, node=s, exists=True ):
                    mgeoSets.append(s)
        if mgeoSets:
    ...
    def writeAttrib(self, nset, dict=None):
        ...
        cmds.setAttr(nset+'.'+self.settingsAttribName, str(ui), type="string" )
    ...
    def writeAttribByName(self, item, widget):
        ...
        if opt:
            ...
            ui = self.getAttribString()
            node[opt] = ui[opt]
            cmds.setAttr(item+'.'+self.settingsAttribName, str(node), type="string" )
        else:
            pass
            # print 'WIDGET NOT FOUND FOR SAVING'

    def getOptionName(self, widget):
        return widgetsMap.widgets.get(widget)
    ...
    def copyParameters(self, item):
        ...
        self.bufer = self.readAttrib(item)
